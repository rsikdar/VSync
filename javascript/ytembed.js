    ...
    listVideos: function(json, cfg) {
        ...
        if (json.error) {
            ...
            for (var i = 0; i < json.items.length; i++) {
                ...
                a.className = 'clip';
                // hashids = new Hashids();
                // console.log(new Hashids("ryan is a bitch"));
                ...
                var finalUrl = 'http://' + location.host + url;
                ...
                if (this.cfg.player == 'embed') {
                    ...
                    //  if (a.addEventListener) {
                    //     a.addEventListener('click', function() {console.log('clicked');}, false);
                    // } else if (a.attachEvent) {
                ...
                if (this.cfg.layout == 'thumbnails') {
                    ...
                    //this.cfg.layout = full
                    li.innerHTML = '<table cellspacing="0" cellpadding="0" border="0"><tr><td valign="top" rowspan="2"></td><td valign="top"><h3><a href=' + finalUrl + '>' + entry.snippet.title + '</a></h3><span>' + this.formatDescription(entry.snippet.description) + '</span></td><td valign="top" style="width: 150px" class="meta"><div>' + (entry.contentDetails ? 'Duration: ' + ytEmbed.formatDuration(entry.contentDetails.duration) + '<br>' : '') + (entry.statistics ? 'Views: ' + entry.statistics.viewCount + '<br>' : '') + 'From: entry.snippet.channelTitle</div></td></tr></table>';
